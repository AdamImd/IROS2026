...
To optimize and evaluate our design, we measure the response of an isolated cell of the sensor across a range of configurations. We use an Instron universal tensile tester to find the resistance of the sensor to a known force, where we apply forces from \textit{0-35N} to a single taxel for five load-unload cycles \ktxt{TODO: UPDATE}. We then fit a \ktxt{MODEL INFO}


\ktxt{Update for new methods: }We use a common 30$\times$30mm$\times$3mm TPU flat inner layer with 2mm ridges to characterize our outer layers. We varied the thicknesses of the outer layers across the range of $[0.5, 1, 2, 3, 5]\ $mm, and tested their response to applied forces.
