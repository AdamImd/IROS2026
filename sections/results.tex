...
The fabric sensor in the flat configuration has ...
...
To optimize and evaluate our design, we measure the response of an isolated cell of the sensor across a range of configurations. We use an Instron universal tensile tester to find the resistance of the sensor to a known force, where we apply forces from \textit{0-\ktxt{TODO}} to a single taxel for five load-unload cycles \ktxt{TODO: UPDATE}. We then fit a \ktxt{MODEL INFO}


\ktxt{Update for new methods: }We use a common 30$\times$30mm$\times$3mm TPU flat inner layer with 2mm ridges to characterize our outer layers. 
We varied the thicknesses of the outer layers across the range of \ktxt{UPDTATE:$[0.5, 1, 2, 3, 5]\ $mm,} and tested their response to applied forces.
...
% \begin{figure*}
%     \centering
%     \includegraphics[width=0.95\columnwidth]{images/power_law/TPU_loading_by_cell_with_threshold.pdf}\\
%     \includegraphics[width=0.95\columnwidth]{images/power_law/Soft_loading_by_cell_with_threshold.pdf}
%     \caption{TOP: The response of the fabric sensor from 1-35 N. BOTTOM: The response of the TPU sensor from 8-35N. We find that the response of the sensor follows a power law relationship between force and resistance. We fit the model to $F=AR^B$ (R=Resistance) The exponent B of the power law represents the sensitivity of the sensor, with larger magnitude indicating higher sensitivity.}
% \end{figure*}
