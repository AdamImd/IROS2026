...
Overall, the textile sensor is configured for low-force interactions, with a lower detection threshold in the flat configuration (Fig.~\ref{fig:overall_comparison}). In contrast, the AM sensor has an elevated detection threshold due to the designed gap between inner and outer shells; as force increases, shell deformation closes this gap and produces a more abrupt resistance transition.
...
\subsubsection{Findings}
Across tested configurations, textile and AM sensors show similar repeatability scores (Fig.~\ref{fig:overall_comparison}, top), indicating comparable cycle-to-cycle variation under matched calibration conditions. However, the AM sensor exhibits higher hysteresis than the textile sensor (Fig.~\ref{fig:overall_comparison}, bottom), which we attribute primarily to viscoelastic effects in the AM shell. The textile sensor shows a smaller separation between loading and unloading curves under these test conditions.
